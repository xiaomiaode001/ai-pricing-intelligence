...
Run a manual-first intelligence workflow for global iOS subscription price gaps. Treat App Store Price as the full-region aggregation source, Apple storefront pages as spot-check evidence, official provider pages only as product and plan naming context, and FX recomputation only as a CNY verification layer.

Default mode is designed for daily use: capture today's product snapshot, generate a full plan matrix, and expand one detail plan with low-price regions, the US baseline, the highest region, FX checks, and Apple spot checks.
...
1. For routine monitoring, prefer the single entrypoint.
...
2. Use accuracy mode only when the default is not enough.

   - `--accuracy-mode light`: App Store Price + FX only; Apple spot check is manual.
...
3. Use report mode only when the daily short report is not enough.
...
4. Today's monitoring snapshot id uses `<product_key>-<app_store_id>-YYYYMMDD`, for example `gemini-6477489729-20260630`.

5. Today's monitoring run automatically applies `config/retention.yml`: keep generated artifacts for 14 days and always keep at least the latest 2 snapshots per product.

6. Use `--cleanup-dry-run` to preview cleanup, or `--skip-cleanup` to keep all generated artifacts for that run.
...
8. Use raw `--url` capture only when the user explicitly provides or authorizes a specific URL. For product monitoring, prefer `--today`.

9. Omit `--plan` for a full product matrix across every detected iOS plan. Use `--plan` only when the user asks for one specific plan.

10. Use `--report-plan` only when the detailed sections should focus on a plan other than the configured default.

11. Use step-by-step scripts only for debugging or compatibility:
...
- `references/field-schema.md`: normalized row fields, FX fields, spot-check fields, parser metadata, and product routing fields.
...
- Use the full 11-module report only when `--report-mode full` or `--report-mode both` is requested: 结论卡片, 套餐发现结果, 单套餐全球概览, Top 10 低价区, Top 5 高价区, 汇率复算摘要, Apple storefront 抽样复核, 与上一快照对比, 信息差判断, 准确性与限制, Sources.
- Default depth is a full product plan matrix plus one detailed plan section.
...
- The product plan matrix must include every detected plan. If a plan tab is detected but no regional price rows were captured, mark the plan row as `needs_snapshot`.
- Preserve provider-specific plan labels from App Store Price. Do not force Gemini or Claude plans into ChatGPT naming.
- Show Apple status as `apple_spot_checked`, `pending_spotcheck`, `conflict`, `spotcheck_failed`, or `needs_snapshot`. Only the detailed report plan is spot-checked by default.
...
- Keep App Store Price CNY as `source_cny_price`; never overwrite it with FX recomputation or Apple spot-check results.
...
- If App Store Price, Apple storefront, or FX recomputation disagree, show values side by side and mark `conflict`.
- Apply information-gap levels:
  - `high`: lowest is more than 15% below the United States baseline, or more than 35% below the highest region.
  - `medium`: lowest is 5% to 15% below the United States baseline.
  - `low`: lowest is less than 5% below the United States baseline.
  - `none`: lowest is close to or above the United States baseline.
